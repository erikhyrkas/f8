...
// exponent.
//
// For our 8-bit exponent, we'll make the bias dynamic. A bias of 8 allows for reasonable accuracy between 0 and 1
...
//
// Super special note on 10000000. When we try to represent numbers less than our bias can handle, something
...
// what happens if we round down? We get 0 0000 000, which
//
// One more note. We technically have one more special case we could handle. Right now, 10000000 represents
// -0. Negative zero is mathematically equivalent to positive zero. It feels terrible to waste this one permutation
// on something that is largely unused (there are some weird cases where people have used -0 for their own evil
// purposes in some libraries, but I'm not inclined to support it.)
//
...
float calculate_bias_range(int bias) {
    ...
    return max_for_bias - min_for_bias;
}
...
//    std::cout << " [" <<"ev: " << encoded_value << " rt: " << round_threshold << "=" << (!prepped_bits && adjusted_exponent >= (round_threshold + quarter_bias)) << " pb:" << !prepped_bits<< "] ";
    const uint32_t special_case_tiny = quarter_bias != 0 && encoded_value && (!prepped_bits && adjusted_exponent >= (round_threshold + quarter_bias)); // -5 works
    const uint32_t special_case = special_case_0_1 || special_case_0_neg || special_case_tiny;
